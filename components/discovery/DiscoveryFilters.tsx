    ...
    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-gray-200/50 dark:border-slate-700/50 shadow-lg relative z-30">
      ...
      <div className="lg:hidden p-4">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 bg-gray-100 dark:bg-slate-700 rounded-xl"
        >
      ...
      <div className={`${isMobileFiltersOpen ? "block" : "hidden"} lg:block p-4 sm:p-6 pt-0 lg:pt-6`}>
      {/* Header - Desktop only */}
      ...
      {/* Mobile Clear button - shown outside collapsible when filters are active */}
      {hasActiveFilters && (
        <div className="lg:hidden px-4 pb-4">
          <button
            onClick={onClear}
            disabled={isLoading || isFetching}
            className="w-full py-2 text-sm text-red-600 hover:text-red-700 font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-1"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
